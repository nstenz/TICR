...
## clau: I think this is were the job begins
my @taxa;
my @genes;
...
if ($input_is_mbsum) { ## if mbsum is provided instead of mrbayes output
	...
	## clau: here we can change that we can use taxa not in all genes -----------------
	# Parse taxa present in each gene, determine which are shared across all genes
	my %taxa;
	foreach my $gene (@genes) {
	...
	## clau: same here, we can change this so that not all genes need to have all taxa------
	# Add taxa present in all genes to analysis
	foreach my $taxon (keys %taxa) {
		if ($taxa{$taxon} == scalar(@genes)) {
			push(@taxa, $taxon);
...
## clau: I think that after this part, we only need to have @taxa and @genes
...
## clau: here input data: @taxa, @genes, @quartets
...
## clau: still preparing input data, but done in multiple servers
## ----------------------------------------------------------------------------------------------
...
## clau: at this point we have the mbsum files -------------------------------------------------
