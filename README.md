...
2. mb-run.pl
	* This script handles the parallelization and execution of MrBayes on all partitions located in the tarball generated by alignment-breakdown.pl. Upon completion, this script will have generated a tarball containing the output from MrBayes for all partitions located in the input tarball. This tarball is used as the input for the next script.
3. bucky-run.pl
	* This script handles the parallelization and execution of BUCKy using the output from MrBayes stored in the tarball generated by mb-run.pl. Upon completion, this script will have generated a tarball containing the output from BUCKy for each possible quartet in the input sequence file. A summary of all concordance factors for each quartet will also be output in a csv file for user convenience.

## alignment-breakdown.pl
### Script Work Flow
...
```
alignment-breakdown.pl input.fa -b 100
```
...
```
alignment-breakdown.pl input.fa -b 100 -f 10000
```
...
| -T, --n_threads | the number of forks ALL hosts running analyses can use concurrently | current number of free CPUs |
...
## mb-run.pl
### Script Work Flow
This script begins by appending the user-specified MrBayes block to each Nexus file representing an MDL partition located in the input tarball created by alignment-breakdown.pl. Once this is complete, the script proceeds to run MrBayes on each MDL partition. The results for each partition are then tarballed and gzipped. The tarballs for each partition are then aggregated into a single tarball.
...
#### Running MrBayes
When running MrBayes, this script utilizes the zipped tarball created by alignment-breakdown.pl as its input. For example, if the you used the following command to partition an alignment:

```
alignment-breakdown.pl chromosome1.fa -b 100 -f 10000
```

the required input for mb-run.pl would be named 'chromosome1.tar.gz'. Additionally, this script requires a file containing a properly formated MrBayes block (specified with -m or --mb-block flag). This MrBayes block contains the actual commands that will dictate how the script calls MrBayes. The following represents an example invocation:

```
mb-run.pl chromosome1.tar.gz -m bayes.txt -o chr1-mb
```
...
### Usage
At the minimum this script only requires the tarball created by mb-run.pl as its input. For example, if the you used the following command to run MrBayes:

```
mb-run.pl chromosome1.tar.gz -m bayes.txt -o chr1-mb
```

the required input for bucky-run.pl would be named 'chromosome1.mb.tar'. The following represents an example invocation:

```
bucky-run.pl chromosome1.mb.tar -o chr1-bucky
```
